...
DEFAULT_CATALOG   = ROOT_DIR+'/CATALOG.csv'
DEFAULT_DATA_HOME = ROOT_DIR+'/data'
...
class SEVIRSequence(Sequence):
    ...
    def __init__(self,
        ...
        if self.start_date: #datetime.strptime(self.catalog.time_utc, '%Y-%m-%d %H:%M:%S')
    ...
    def _open_files(self,verbose=True):
        ...
        for f in hdf_filenames:
            ...
            print(f)
            print(self.sevir_data_home)    
            #self._hdf_files[f] = h5py.File(self.sevir_data_home+'/'+f,'r')
            import s3fs
            fs = s3fs.S3FileSystem(anon=True)
            #with fs.open(self.sevir_data_home+'/'+f, 'rb') as data_file:
            data_file = fs.open(self.sevir_data_home+'/'+f, 'rb')
            self._hdf_files[f] = h5py.File(data_file)
